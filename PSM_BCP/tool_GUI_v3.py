...
def Clicked_Analyze():
    ...
    global PeakVariance, frame_Result_02, setsigma, peakdata, name, rawPeakData
    global Option_Var_02
    # Parameter
    Beamline = Option_Var_01.get()
    FileDir = e_01.get()
    if (len(FileDir)<2):
        response = messagebox.showerror("Error!","You didn't input the File Directory!")
    else:
        ...
        if Beamline =='9A':
            FileIndexTestMin = ("%04d" %(FileNumStart))
            FileIndexTestMax = ("%04d" %(FileNumEnd))
        else:
            FileIndexTestMin = ("%05d" %(FileNumStart))
            FileIndexTestMax = ("%05d" %(FileNumEnd))
            
        SelectData = "*_"+FileIndexTestMin+"*"
        OtherwayData1 = find(SelectData, FileDir)
        print(OtherwayData1)
        SelectData = "*_"+FileIndexTestMax+"*"
        OtherwayData2 = find(SelectData, FileDir)
        print(OtherwayData2)

        
        if (len(OtherwayData1)<1 or len(OtherwayData2)<1):  


            response = messagebox.showerror("Error!","No data or Out of File Index Range")
        else:
            ...
            for FileIndex in range(FileNumStart,FileNumEnd+1,IntervalFile):
                ...
                for i in peaks:
                    print(rawPeakDataTemp)
                    print(x2[i])
                    rawPeakDataTemp = np.hstack((rawPeakDataTemp,x2[i]))
                    rawPeakDataTemp = np.hstack((rawPeakDataTemp,y2[i]))
                print(rawPeakDataTemp)
                if (n == 1):
                    rawPeakData = rawPeakDataTemp
                else:
                    rawPeakData = np.vstack((rawPeakData,rawPeakDataTemp))
...
def TestWhereTis():
    ...
    WhereT_Result = Label(frame_13, text= ThisT_text)
    WhereT_Result.pack()

def saveClicked():
    global e_02, peakdata, rawPeakData, Option_Var_01, Option_Var_02
    Save_Path = e_02.get()
    if (len(Save_Path)<2):
        response = messagebox.showerror("Error!","You didn't input the Save Directory!")
    else:

        e = datetime.datetime.now()
        ExportFileName = ("%s%02d%02d_data_%02d%02d%02d.csv" % (e.year, e.month, e.day, e.hour, e.minute, e.second))
        Save_Path = Save_Path + "/" + ExportFileName
        Save_Path = Save_Path.replace('\\','/')
        print(Save_Path)
        print(Option_Var_01.get())
        print(Option_Var_02.get())
        if Option_Var_02.get() =="Raw":
            pd.DataFrame(rawPeakData).to_csv(Save_Path)
        else:
...
e_01.pack(pady=(0,10))
e_01.insert(0,r"C:\Users\taesu\Documents\GitHub\RDF_from_X-ray_Scattering\PSM_BCP")
...
e_02.pack(pady=(0,10))
e_02.insert(0,r"C:\Users\taesu\Documents\GitHub\RDF_from_X-ray_Scattering\PSM_BCP")
